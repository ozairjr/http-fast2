const mountRouter = () => {
    ...
    const mapVirtualRoutes = (routesFilePath) => {
        ...
        routes.forEach(routeArr => addRoute(routeArr[0], routeArr[1], routeArr.length > 2 ? routeArr[2] : undefined))
    }
    ...
    const process = (requestParams, request, response) => {
        try {
            ...
            let isItOK = middlewares.reduce((flag, middFnc) => {
            ...
            if (request.queryString && request.queryString.files) {
                Object.keys(request.queryString.files).forEach((key) => {
                    ...
                    try {
                        ...
                        console.log('[ERROR] Falha ao apagar arquivo temporario:', file && file.getName())
                    }
                })
            }
        }
